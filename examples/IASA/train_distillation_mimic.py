...
class TextVideoDataset_train(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.attention_type == "multiregion_causal":
...
class LightningModelForTrain_onestage(pl.LightningModule):
    def __init__(
        ...
        vae_model_manager = ModelManager(torch_dtype=torch.bfloat16, device="cpu")
        vae_model_manager.load_models(model_path)
        self.pipe_VAE = WanVideoPipeline.from_model_manager(vae_model_manager)
        self.tiler_kwargs = {"tiled": tiled, "tile_size": tile_size, "tile_stride": tile_stride}
            
        self.pipe_VAE = self.pipe_VAE.eval()
